  ...
  return (
    <div className="grid grid-cols-1 md:grid-cols-2 gap-6 h-full overflow-y-auto pr-2 pb-20">
      {fields.map(({ key, label, placeholder }) => (
        <div key={key} className="flex flex-col h-full min-h-[200px] bg-white rounded-xl border border-gray-200 shadow-sm focus-within:ring-2 focus-within:ring-[var(--color-kanji)]/20 focus-within:border-[var(--color-kanji)] transition-all">
          <div className="bg-gray-50/50 px-4 py-2 border-b border-gray-100 flex items-center gap-2">
            <span className="w-6 h-6 rounded-md bg-[var(--color-kanji)]/10 text-[var(--color-kanji)] font-bold flex items-center justify-center text-xs uppercase">
              {key.toUpperCase()}
            </span>
          ...
          <TextareaAutosize
            ...
            className={`flex-1 w-full p-4 resize-none outline-none text-sm text-gray-700 leading-relaxed placeholder:text-gray-300 ${isDiscreteMode ? 'blur-sm' : ''}`}
            minRows={5}
          />
        </div>
      ))}
