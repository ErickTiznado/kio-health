...
export function TemplateSelector({ currentType, onSelect, hasContent }: TemplateSelectorProps) {
  ...
  const handleSelect = (type: NoteTemplateType) => {
    ...
    if (hasContent) {
      if (confirm('Cambiar de plantilla podría alterar el contenido actual. ¿Continuar?')) {
        onSelect(type);
      }
    } else {
      onSelect(type);
      ...
      <AnimatePresence>
        {isOpen && (
          <motion.div
            ...
            className="absolute top-full left-0 mt-2 w-56 bg-white dark:bg-slate-900 rounded-xl shadow-xl border border-gray-100 dark:border-slate-800 p-1.5 z-50 overflow-hidden"
          >
