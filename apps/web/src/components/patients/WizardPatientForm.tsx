...
import { Check, AlertCircle, Calendar, User, Phone, HeartPulse, FileText, ArrowLeft, ArrowRight } from 'lucide-react';
...
export function WizardPatientForm({ initialData, onSubmit, onCancel, isLoading }: WizardPatientFormProps) {
  ...
  const activeInputClass = "border-kio dark:border-kio";
                ...
                <div key={s.id} className="flex items-center">
                    <div 
                        className={`w-2.5 h-2.5 rounded-full transition-all duration-500 ${
                            step >= s.id ? 'bg-kio scale-110' : 'bg-gray-200 dark:bg-slate-700'
                        }`} 
                    />
                    {s.id < STEPS.length && (
                        <div className={`w-8 h-0.5 mx-1 transition-all duration-500 ${
                             step > s.id ? 'bg-kio' : 'bg-gray-200 dark:bg-slate-700'
                        }`} />
                    )}
                </div>
            ))}
