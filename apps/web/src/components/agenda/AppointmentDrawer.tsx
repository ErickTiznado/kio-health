...
import { X, Play, CreditCard, FileText, Clock, Stethoscope, ClipboardList, Calendar, Edit2, Ban, User, Check, Hourglass, ExternalLink, Video, ChevronDown } from 'lucide-react';
...
const PAYMENT_CONFIG: Record<string, { label: string; className: string; icon: ReactNode }> = {
  PENDING: { label: 'Pendiente', className: 'bg-amber-50 text-amber-700 border border-amber-200', icon: <Hourglass size={12} /> },
  PAID: { label: 'Pagado', className: 'bg-emerald-50 text-emerald-700 border border-emerald-200', icon: <Check size={12} /> },
};
...
export function AppointmentDrawer({ appointment, isOpen, onClose }: AppointmentDrawerProps) {
  ...
  // Local state for payment status to demonstrate immediate feedback (Phase 2)
  ...
  if (appointment && appointment.paymentStatus !== paymentStatus) {
  ...
  const paymentInfo = PAYMENT_CONFIG[paymentStatus] ?? PAYMENT_CONFIG.PENDING;
  const typeLabel = TYPE_LABELS[appointment.type] ?? appointment.type;

  // Relative time context (Phase 2)
  ...
  // Check for history empty state (Phase 2)
  // We'll use a mocked empty state check logic for demonstration
  const history = []; // Force empty for demo if needed, or use MOCK_HISTORY
  // const history = MOCK_HISTORY; // Uncomment to see data
        ...
        <div className="relative px-6 pt-6 pb-5 bg-[var(--color-bg)]">
          ...
          <div className="flex items-center gap-4 mt-6">
            ...
            <div className="flex-1 min-w-0">
              ...
              {/* Micro-Profile Header (Miller's Law) */}
              <div className="flex items-center gap-2 text-xs text-gray-600 mt-1 font-medium">
                  <span>{MOCK_PATIENT_EXTENDED.age} años</span>
                  <span className="w-0.5 h-0.5 rounded-full bg-gray-400" />
                  <span>{MOCK_PATIENT_EXTENDED.gender}</span>
                  <span className="w-0.5 h-0.5 rounded-full bg-gray-400" />
                  <span>Desde {MOCK_PATIENT_EXTENDED.patientSince}</span>
              </div>
               ...
               <div className="flex items-center gap-2 mt-2.5">
                <span className={`inline-flex items-center gap-1.5 text-[10px] font-bold px-2.5 py-0.5 rounded-md ${statusInfo.className} border border-transparent`}>
                 ...
                 <span className="text-[10px] items-center gap-1 flex text-gray-500 bg-white/50 px-2 py-0.5 rounded-md border border-gray-100">
                    <User size={10} />
                    {MOCK_PATIENT_EXTENDED.occupation}
                 </span>
              </div>
            </div>
          </div>
        ...
        <div className="bg-gray-50/50 border-t border-gray-100 p-6 space-y-4">
             ...
             <div>
                ...
                <div className="flex items-center justify-between bg-white rounded-xl p-3 border border-gray-200 shadow-sm">
                    <p className="text-sm font-bold text-gray-800">${appointment.price} MXN</p>
                    
                    <button 
                        onClick={togglePayment}
                        className={`text-xs font-bold pl-3 pr-2 py-1.5 rounded-lg flex items-center gap-2 transition-all hover:bg-opacity-80 active:scale-95 ${paymentInfo.className} cursor-pointer select-none`}
                    >
