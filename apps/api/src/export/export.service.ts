  ...
  async generateSessionPdf(
    ...
    return new Promise((resolve, reject) => {
      ...
      if (appointment.patient.dateOfBirth) {
        doc.text(
          `Fecha de Nacimiento: ${appointment.patient.dateOfBirth.toLocaleDateString()}`,
        );
      }
      doc.moveDown();
      ...
      doc.text(
        `Fecha: ${appointment.startTime.toLocaleDateString()} ${appointment.startTime.toLocaleTimeString()}`,
      );
...
${content.p}`,
          ...
          doc.text(
            content.body || content.notes || 'Sin contenido estructurado.',
          );
        }
        ...
        if (includePrivateNotes && appointment.psychNote.privateNotes) {
          doc
            .fontSize(14)
            .fillColor('red')
            .text('Notas Privadas (Confidencial)', { underline: true });
          doc
            .fillColor('black')
            .fontSize(12)
            .text(appointment.psychNote.privateNotes); // Assumes already decrypted passed in
          doc.moveDown();
        }
      } else {
